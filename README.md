...
- `recommendation_vector` — отдельный вектор для подбора заданий
- `selected_tasks`
- `selected_theory`
- `workbook.items`
- `workbook.latex`
...
3. Отдельно строится `recommendation_vector`, который отличается от общего рейтинга.
   Он нужен именно для подбора заданий.
4. Для него используются:
   - `mastery_gap = 1 - mastery`
   - `recent_error_rate` по последним попыткам
   - `recency_factor` по давности последней попытки
   - `practice_gap` по дефициту практики
   - `trend_penalty`, если недавние попытки хуже общего mastery
5. Предметные priors тэгов берутся из MongoDB.
6. `recommendation_vector` считается по формуле:

`score = prior_weight * (0.35*mastery_gap + 0.25*recent_error_rate + 0.20*recency_factor + 0.15*practice_gap + 0.05*trend_penalty)`

Это значит, что рекомендационный вектор может осознанно отличаться от общего `mastery`:

- тема может быть формально "знаемой", но давно не практиковаться
- тема может иметь хороший общий рейтинг, но плохой недавний тренд
- тема может быть недоотработана просто из-за малого числа попыток

7. `recommendation-service` запрашивает задачи и теорию у `content-service`.
8. Задачи ранжируются по:
   - близости к `recommendation_vector`
   - слабости соответствующих тем
   - относительной сложности из `course calibration`
9. После выбора задач сервис подбирает теорию по темам этих задач.
10. На выходе строится готовая рабочая тетрадь и LaTeX-документ.
